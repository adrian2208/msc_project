...
Each point on the $M$- dimensional lattice is assigned a global index based on its global coordinate, given by 
\begin{equation}
...
Because the global parameterization is shared across all processes, there is no ambiguity as to what lattice point is being addressed.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[scale = 0.7]
...
Therefore, the lattice should be split along as few dimensions as possible, and only when $N_{procs}$ exceeds the size of the lattice in the first dimension, should the next dimension be included in the subdivision.\\An example of the former can be seen in figure \ref{figure:Local_lattice_parameterization} where we imagine the lattice and process domains extending in two more dimensions, yet each process will only boarder two other processes. This of-course leads to terrible scaling, as the lattice can maximally by partitioned into 6 processes. Implementing this partitioning scheme is trivial, and so we move on to the latter case of subdividing along two dimensions.
\paragraph{Space-Time-French-Fry}
