...
\author{A.\ M.\ Kleven}
\begin{document}
\uiomasterfp[nosp, program={Computational science: physics},supervisors={Prof. A. Shindler\and Prof. M. Hjort-Jensen},title={Applications of the Yang-Mills gradient flow to topological observables in QCD},dept={Department of physics},color=grey]
...
\listoftables{}
\chapter*{Abstract}
We study the topological charge and its distribution in $SU(3)$ Yang-Mills theory using the highly improved lattice field strength tensor and the Wilson flow. The topological susceptibility is estimated to high accuracy by considering a large range of lattice spacings and by keeping finite size effects negligible, with a continuum result of $r_0^4\chi_t = 0.0613(20).$ This result is consistent with results obtained using chiral lattice Dirac operators and the spectral projector method, supporting the conjecture that the moments of the flowed topological charge coincide with those in the chiral Ward identities.\\\\A universality test is conducted on the dependence of the topological susceptibility on topological charge definitions, testing the highly improved v. the clover definition of the topological charge. The results are consistent with universality.\\Another such study on the universality of the topological susceptibility defined at different flow times was conducted with results compatible with universality.\\An attempt is made to determine the non-gaussianity of the topological charge distribution, but large statistical errors make the analysis inconclusive as to the predictions made by the large $N_c$-expansion and the dilute instanton gas model.\\\\A preliminary study as to the feasibility of estimating the $\Lambda_{YM}$-scale is carried out with promising results. One set of results are seen to be consistent with estimates made using different methods, producing estimates in the $68$th- percentile confidence interval between $224.96MeV$ and $240.15MeV$. However these results are undermined with a hereto unmanaged systematic error related to the choice of fit domain.
...
This thesis has been formed on the basis of many pre-existing analyses. The determination of the topological susceptibility has been done in many instances. The particular result obtained here using the improved lattice field strength is as far as we can tell, the most accurate result of it's type if it can be verified. The near absence of any scaling effects is particularly promising.\\That said, there are some results here which have not been found in other papers\footnote{Note that these results may well exist, but have not been found in the course of researching this thesis}. Two universality tests have been conducted which demonstrate the universality of the topological susceptibility defined from the improved lattice field strength tensor at different flow times as well as the universality of the topological susceptibility defined from the clover- and Improved- definitions of the lattice field strength tensor.\\\\Besides a results point of view, we hope that this thesis provides a coherent and relatively self contained guide to understanding the gradient flow as applied to topological observables, both theoretically and practically.\\\\This text is of course only part of the substantial undertaking that was this project. This is after all a thesis in computational physics and with that comes the computational part. 'Lattice Gradient Flow' or LGF\footnote{The LGF program can be found at the Github address \url{https://github.com/adrian2208/msc_project}} is a suite of tools created in the $C++$ programming language with the intention of providing a framework for doing high performance computing of lattice QCD and the gradient flow. 
\section{Structure}
...
\caption[Difference between topological charge definitions]{Expectation value of the difference between the two topological charge definitions when measuring the same configuration. The measurements are taken at $t/a^2 = \{13,\,8.5,\,5.5,\,3.5 \}$ from left to right.}\label{fig:Q_FST_defs_diff}
\end{figure}
...
\caption[Ratio of differently defined energy densities]{Ratios of the expectation values of the two Energy density definitions ((I)mproved \& (C)lover) when measuring the same configurations. The measurements are taken at $t/a^2 = \{13,\,8.5,\,5.5,\,3.5 \}$ from left to right.}\label{fig:E_FST_defs_diff}
\end{figure}
\FloatBarrier
...
\caption[Flow time dependence of the energy density]{Expectation value of the energy density as a function of the physical flow time over the square of the Sommer parameter. The error bars are smaller than the symbols. }\label{fig:EvFlow}
\end{figure}
...
\caption[Flow time dependence of $t^2\langle E\rangle$]{The dimensionless combination $t^2\langle E\rangle$ as a function of the physical flow time over the squared Sommer radius. The errorband in the perturbative expansion is dominated by the uncertainty in $\Lambda\big|_{N_f=0}$ which is sited in \cite{CapitaniStefano1999Nqmr}. The errorbars are not visible on the scale of the figure.}\label{fig:tsquaredE_v_flowtime}
\end{figure}
...
\caption[Continuum extrapolation of the reference scale $t_0$]{Linear extrapolation in $a^2/t_0$ of $\frac{\sqrt{8t_0}}{r_0}$ to the continuum. The horizontal error is not visible on the scale of the figure.}\label{fig:SmearRad_v_latticeConstant}
\end{figure}
...
\caption[$t^2\left\langle E\right\rangle $ dependence on the momentum scale]{$t^2\left\langle E\right\rangle $ dependence on the momentum scale $q = \frac{1}{\sqrt{8t}}[MeV]$ with the extrapolation to the continuum based on the three finest lattice spacings.}\label{fig:Coupling_cont_extrapolation}
\end{figure}
...
\caption[Flow time dependence of the topological charge]{Flow time dependence of the topological charge going from coarsest to finest lattice spacing from top to bottom. $\frac{t_0}{a^2}$ corresponds to the computer- flow time at which the topological susceptibility is measured. 21 samples of each ensemble are illustrated.}\label{fig:QvFlowTime}
\end{figure}
...
\end{subfigure}
\caption[Spatial distribution of the topological charge density at different flow times]{Topological charge distribution at euclidean time slice $t_{euc}=0$ measured at different flow times. Generated using the Mayavi python plotting utility \cite{ramachandran2011mayavi}. The star shape is only an artefact of the 2D projection.}
...
\caption[flow time dependence of the topological susceptibility]{Fourth root of the topological susceptibility in physical units as a function of $\frac{\sqrt{8t}}{r_0}$}\label{fig:FlowTime_v_smearRad}
\end{figure}
...
\caption[$R$ continuum limit]{Linear (red) and constant (black) continuum extrapolation of $R$ using data from every lattice spacing. Both predictions by the dilute instanton gas model ($R=1$) and the large $N_c$- expansion ($\mathcal{O}(R) = \mathcal{O}(\frac{1}{N^2})$) are easily within the errorbars.}\label{fig:R-contLim}
\end{figure}
