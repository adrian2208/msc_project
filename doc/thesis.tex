...
file:///C:/Users/adria/Documents/Studier/Master/Special%20Syllabus/L%C3%BCscher1982_Topology_Of_Lattice_Gauge_Fields.pdf
...
As it happens, we just introduced the 1-form $A_{\mu}(x)$, and so applying the covariant to it, doesn't seem like such a big leap. We get that
...
\end{subfigure}
\caption{$C^{(1,1)}$ and $C^{(2,2)}$ contain a factor of $\frac{1}{4}$ since the two terms are symmetric under $n\leftrightarrow m$. $C^{(1,2)}$ and $C^{(1,3)}$ each need to be evaluated for two different orientations and their results added and multiplied by a factor of $\frac{1}{8}$}\label{figure:Clovers}
\end{figure}
...
It's not immediately obvious how one should interpret an equation such as 
\begin{equation}
...
\end{equation}
because we're not just dealing with an 8- dimensional Lie group, but rather the $8\cdot4\cdot\left| \Lambda \right|$ dimensional Lie group $$\mathcal{G} =\prod\limits_{x\in \Lambda ,\,\mu}\!\!\!G_{\mu,x}$$ denoting the direct product of the $SU(3)$ group associated with every link of the lattice.
\begin{equation}
\dot{V}_{t}=Z\left(V_{t}\right) V_{t}
\end{equation}
\begin{equation}
\begin{aligned}
W_{0} &=V_{t} \\
...
CITE THE MATRIX PROCESSING PAPER YOU BASED THE LATTICE REPRESENTATION ON
\subsection{Parallelization}

\subsection{Checkerboard Update}
