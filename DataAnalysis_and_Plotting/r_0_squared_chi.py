...
def calc_aOverr0W_errors(beta):
    ...
    return out
#DO NOT COPY!!!!!!!!!!!!!!!!!!!!!!!!!! \/ \/ \/ \/ \/ \/
def TopSusc_for_regplot(topChargeArray,aOverR0,latticeSize):#DO NOT COPY THIS FUNCTION - ITS OUTPUT IS ONLY MEANT TO WORK FOR SNS.REGPLOT
    df = topChargeArray**2
    ...
    temp = (temp/(latticeSize*aOverR0**4))
...
for j in range(len(directory_list)):
    ...
    temp = TopSusc_for_regplot(df2,aOverR0_list[j],V[j])
...
a_axis = np.linspace(-0.0005,0.035,100)
...
plt.ylabel(r'$r_0^4\chi_t$')
#plt.legend()
plt.savefig('r_0chi.pdf', bbox_inches="tight")
