...
def RatioCovariance(topChargeArray,n_resamples,sampleSize):
    Q_squared = topChargeArray**2
    Q_fourth = topChargeArray**4
    means = []
    vars = []
    for i in range(n_resamples):
        sample_squared = np.array(random.sample(list(Q_squared),sampleSize))
        sample_fourth = np.array(random.sample(list(Q_fourth),sampleSize))
        #vars.append(np.var(sample1/sample2))
        temp_denom = np.mean(sample_squared)
        temp_enum = np.mean(sample_fourth) - 3*temp_denom**2
        means.append(temp_enum/temp_denom)
        #print(sample1)
        #print(sample2)
...
for j in range(len(directory_list)):
    ...
    mean, std = TopSusc_for_regplot(df2,8000,fileEnd_list[j]-50)
...
plt.ylabel(r'$R$')
#plt.legend()
plt.savefig('R_continuum_extrapolation-SmallerErrors.pdf', bbox_inches="tight")
