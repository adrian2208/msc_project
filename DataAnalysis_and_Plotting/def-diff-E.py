...
for j in range(len(dirs)):
    ...
    for i in range(fileStart,fileEnd+1,1):
        ...
        frames.append(df1)
    df2 = pd.concat(frames,axis=1).iloc[-1].to_numpy()
    ...
    mean,std = calcE(df2)#RatioCovariance(df2**2,df2_unimproved**2,15000,(fileEnd_list[j]-fileStart_list[j])-5)
    mean2,std2 = calcE(df2_unimproved)
    temp = ufloat(mean,std)/ufloat(mean2,std2)
    meanList.append(temp.nominal_value)
...
plt.ylabel(r'$\frac{\left\langle E_I \right\rangle}{\left\langle E_C \right\rangle}$',rotation = 0,labelpad=18)
