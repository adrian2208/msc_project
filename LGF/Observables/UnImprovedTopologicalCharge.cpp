#include "UnImprovedTopologicalCharge.h"
#include <fstream>
#include <algorithm>
UnImprovedTopologicalCharge::UnImprovedTopologicalCharge(SU3_field& U) {
	m_U = &U;
}

void UnImprovedTopologicalCharge::calculate(double flowTime) {
	...
	for (int i = (*m_U).Responsible_Start(); i < (*m_U).Responsible_Stop(); i++) { //This loop, alongside the multiple of eight constitutes the 4D levi cevita, thanks to the cyclicity of the trace 
		localSum += (((*m_U).FieldStrengthTensor(i, 3, 2)) * ((*m_U).FieldStrengthTensor(i, 0, 1))).ReTr();
		localSum += (((*m_U).FieldStrengthTensor(i, 3, 1)) * ((*m_U).FieldStrengthTensor(i, 2, 0))).ReTr();
		localSum += (((*m_U).FieldStrengthTensor(i, 3, 0)) * ((*m_U).FieldStrengthTensor(i, 1, 2))).ReTr();
	}
...
void UnImprovedTopologicalCharge::saveTopologicalChargeToFile(double beta, const std::string& updateMethod, const std::string& dataFolder, const std::string& identifier) {
	if (mpiWrapper::id() == 0) {
		std::string beta_str = std::to_string(beta);
		std::replace(beta_str.begin(), beta_str.end(), '.', '_');
		n_fs::path fieldType("/UnImprovedTopological_Charge/beta" + beta_str + "/");
