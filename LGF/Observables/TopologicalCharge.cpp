...
void TopologicalCharge::calculate(double flowTime) {
	...
	for (int i = (*m_U).Responsible_Start(); i < (*m_U).Responsible_Stop(); i++) { //This loop, alongside the multiple of eight constitutes the 4D levi cevita, thanks to the cyclicity of the trace 
		...
		//localSum += ((*m_U).clover_avg(i, 3, 2) * (*m_U).clover_avg(i, 0, 1)).ReTr();
		//localSum += ((*m_U).clover_avg(i, 3, 1) * (*m_U).clover_avg(i, 2, 0)).ReTr();
		//localSum += ((*m_U).clover_avg(i, 3, 0) * (*m_U).clover_avg(i, 1, 2)).ReTr();

		//localSum += (((*m_U).clover_avg(i, 3, 2) + (*m_U).RectangleClover_avg(i, 3, 2)) * ((*m_U).clover_avg(i, 0, 1) + (*m_U).RectangleClover_avg(i, 0, 1))).ReTr();
		//localSum += (((*m_U).clover_avg(i, 3, 1) + (*m_U).RectangleClover_avg(i, 3, 1)) * ((*m_U).clover_avg(i, 2, 0) + (*m_U).RectangleClover_avg(i, 2, 0))).ReTr();
		//localSum += (((*m_U).clover_avg(i, 3, 0) + (*m_U).RectangleClover_avg(i, 3, 0)) * ((*m_U).clover_avg(i, 1, 2) + (*m_U).RectangleClover_avg(i, 1, 2))).ReTr();
		...
		//localSum += (((*m_U).clover_avg(i, 0, 1) + (*m_U).RectangleClover_avg(i, 0, 1)) * ((*m_U).clover_avg(i, 2, 3) + (*m_U).RectangleClover_avg(i, 2, 3))).ReTr();
		//localSum -= (((*m_U).clover_avg(i, 0, 2) + (*m_U).RectangleClover_avg(i, 0, 2)) * ((*m_U).clover_avg(i, 1, 3) + (*m_U).RectangleClover_avg(i, 1, 3))).ReTr();
		//localSum += (((*m_U).clover_avg(i, 0, 3) + (*m_U).RectangleClover_avg(i, 0, 3)) * ((*m_U).clover_avg(i, 1, 2) + (*m_U).RectangleClover_avg(i, 1, 2))).ReTr();

		//localSum += ((identity-(*m_U).plaquette(i, 0, 1)) * (identity - (*m_U).plaquette(i, 2, 3))).ReTr();
		//localSum -= ((identity - (*m_U).plaquette(i, 0, 2)) * (identity - (*m_U).plaquette(i, 1, 3))).ReTr();
		//localSum += ((identity - (*m_U).plaquette(i, 0, 3)) * (identity - (*m_U).plaquette(i, 1, 2))).ReTr();
		
		//localSum += (((*m_U).Plaquette_FieldStrengthTensor(i, 0, 1)) * ((*m_U).Plaquette_FieldStrengthTensor(i, 2, 3))).ReTr();
		//localSum -= (((*m_U).Plaquette_FieldStrengthTensor(i, 0, 2)) * ((*m_U).Plaquette_FieldStrengthTensor(i, 1, 3))).ReTr();
		//localSum += (((*m_U).Plaquette_FieldStrengthTensor(i, 0, 3)) * ((*m_U).Plaquette_FieldStrengthTensor(i, 1, 2))).ReTr();
