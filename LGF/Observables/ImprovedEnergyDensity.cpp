#include "ImprovedEnergyDensity.h"
#include <fstream>
#include <algorithm>

ImprovedEnergyDensity::ImprovedEnergyDensity(SU3_field& U) {
	m_U = &U;
}

void ImprovedEnergyDensity::calculate(double flowTime) {
...
void ImprovedEnergyDensity::saveEnergyDensityToFile(double beta, const std::string& updateMethod, const std::string& dataFolder, const std::string& identifier) {
	if (mpiWrapper::id() == 0) {
		std::string beta_str = std::to_string(beta);
		std::replace(beta_str.begin(), beta_str.end(), '.', '_');
		n_fs::path fieldType("/ImprovedEnergyDensity/beta" + beta_str + "/");
